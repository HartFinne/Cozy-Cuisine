...
@onready var player_girl: CharacterBody2D = $"../player_girl"
...
func _process(delta: float) -> void:
	...
	var distance = position.distance_to(player_girl.position)
	if distance < TRIGGER_DISTANCE and Input.is_action_just_pressed("accept"):
		...
		if player_data:
			player_data.player_position = player_girl.global_position
			player_data.save()
